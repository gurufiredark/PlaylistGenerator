...
from functions.util import getSongTitlesFromFile, inicializarObjetosSong, printarMusicas, printarMusicasPopulation
...
for generation in range(GENERATIONS):
    ...
    fitness_scores = [fitness_function(individual, playlists) for individual in population]
    ...
    population = sorted(population, key=lambda x: fitness_function(x, playlists), reverse=True)[:POPULATION_SIZE]
...
for playlist in playlists:
    best_individual = max(population, key=lambda x: fitness_function(x, [playlist]))
